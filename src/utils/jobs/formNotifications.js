...
export function setFormNotifications() {
    new CronJob(
        '*/1 * * * *',
        async function () {
            const gdanskShiftsChannel = await client.channels.fetch(client.config.gdanskShiftsChannelId);
            const gdyniaShiftsChannel = await client.channels.fetch(client.config.gdyniaShiftsChannelId);
